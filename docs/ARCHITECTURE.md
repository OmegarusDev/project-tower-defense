...
| `AttackPlan` | `(base,barrel,payload,level,opts)` → pattern/damage/aura; auto-level + branch ranks; base×barrel range/ROF mults stack with Arsenal ranks |
| `combat` | Targeting, auras, projectiles (pierce / ballistic / homing coast), pulses, status, XP→auto-level + pending branch picks |
