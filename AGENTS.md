...
**Campaign:** **shipped slice = 12 linear Act I levels** (was 7 — expanded, see `data/campaign.js`); future PINNED 5/10/25 × 3 campaigns. Prep = compose+Forge+Aether (untimed); in-level roster frozen (no compose/meta); predefined map seeds + valid pre-walls; victory per level data (waves and/or boss).
