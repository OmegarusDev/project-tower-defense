...
// ——— C1: between-wave vs in-wave continue math (via real Sim checkpoint) ———
{
  ...
  if (blobIn.phase !== "betweenWaves" && (blobIn.wave | 0) > 0) r1.state.waves.index = (blobIn.wave | 0) - 1;
  assert(r1.state.waves.index === 3, "inWave continue rolls back");
  r1.startWave();
  assert(r1.state.waves.index === 4, "Call restarts saved wave");

  // betweenWaves: keep cleared index; Call starts next
  const sim2 = new Sim();
  sim2.setup(11, 14, 21, true);
  sim2.startWave();
  sim2.state.checkpointPhase = "betweenWaves";
  const blobBw = sim2.checkpoint();
  const r2 = new Sim();
  r2.loadCheckpoint(blobBw);
  if (blobBw.phase !== "betweenWaves" && (blobBw.wave | 0) > 0) r2.state.waves.index = (blobBw.wave | 0) - 1;
  assert(r2.state.waves.index === 1, "betweenWaves keeps cleared wave");
  r2.startWave();
  assert(r2.state.waves.index === 2, "Call starts next wave");
}

{
  ...
  assert(sim2.state.waves.index === 3, "load keeps cleared wave");
  assert(sim2.state.towers.length === 1 && sim2.state.towers[0].base === t.base, "towers restored");
  assert(sim2.state.checkpointPhase === "betweenWaves", "phase restored");
}

{
  ...
  assert(sim.state.checkpointPhase === "inWave", "wave start → inWave");
  assert(sim.state.waves.index === 1, "first wave");
  const blob = sim.checkpoint();
...
// ——— C3: Call Early double-dip ———
{
  ...
  sim.state.economy.battle = 100;
  const r1 = sim.startWave({ earlyBonus: 5 });
  assert(r1.earlyBonus === 5, "first early bonus applied");
  assert(sim.state.economy.battle === 105, "battle +5");
  assert(sim.state.earlyBonusWave === 1, "tracks claimed wave");
  const blob = sim.checkpoint();

  const sim2 = new Sim();
  sim2.loadCheckpoint(blob);
  // Continue mid-wave rolls back index in app; simulate that:
  sim2.state.waves.index = (blob.wave | 0) - 1;
  const before = sim2.state.economy.battle;
  const r2 = sim2.startWave({ earlyBonus: 5 });
  assert(r2.earlyBonus === 0, "Continue + Call skips already-claimed early bonus");
  assert(sim2.state.economy.battle === before, "no second early Coin");
  assert(sim2.state.waves.index === 1, "restarts same wave");
}
...
// ——— C2: delta-merge meta gains (the real mergeRunGains, not a re-implementation) ———
{
  ...
  sim.state.metaAppliedGains = { parts: 0, aether: 0 };
  ...
  assert(sim.state.metaAppliedGains.parts === 6 && sim.state.metaAppliedGains.aether === 4, "cursor aligned");
  assert(sim.state.economy.forge === 16 && sim.state.economy.aether === 24, "sim vault aligned");
  ...
  sim.state.economy.runWaveGains.parts = 9;
  mergeRunGains(meta, sim);
...
// ——— C4: rail pierce ———
{
  ...
  const done1 = onHit(sim.state, p, e1);
  ...
  const done2 = onHit(sim.state, p, e2);
  assert(done2 === true, "pierce exhausted → despawn");
  assert(e2.hp < 100, "second enemy damaged");
  assert(onHit(sim.state, p, e1) === false, "skip already-hit id");
}
...
// ——— H1: XP auto-levels; branch picks via tryChooseLevelBranch ———
{
  ...
  t.xpToPoint = 5;
  for (let i = 0; i < 12; i++) grantXp(sim.state, t, 1);
